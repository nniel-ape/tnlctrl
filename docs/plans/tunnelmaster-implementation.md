# TunnelMaster Implementation Plan
...
- Single target: `TunnelMaster` (macOS app)
- Bundle ID: `nniel.TunnelMaster`
...
TunnelMaster/
├── App/
│   ├── TunnelMasterApp.swift      # Menu bar app entry
...
TunnelMasterHelper/                 # Separate target
...
**Files to touch:**
- `TunnelMaster/TunnelMasterApp.swift` — Replace WindowGroup with MenuBarExtra
- `TunnelMaster/App/AppState.swift` — Create (new file)
- `TunnelMaster/Views/MenuBar/MenuBarView.swift` — Create (new file)
...
3. Create `MenuBarView.swift` with basic dropdown:
   ...
   - "Quit" button
4. Update `TunnelMasterApp.swift`:
   ```swift
   @main
...
**Files to touch:**
- `TunnelMaster/Models/Service.swift` — Create
- `TunnelMaster/Models/RoutingRule.swift` — Create
- `TunnelMaster/Models/TunnelConfig.swift` — Create
- `TunnelMaster/Models/Protocol.swift` — Create (enum for proxy protocols)
...
**Testing:**
- Write unit tests in `TunnelMasterTests/`:
...
**Files to touch:**
- `TunnelMaster/Services/KeychainManager.swift` — Create
...
3. Create `KeychainManager.swift`:
   ...
   actor KeychainManager {
       ...
       private let keychain = Keychain(service: "nniel.TunnelMaster")
...
- Keychain Access.app shows entries under "nniel.TunnelMaster"
...
**Files to touch:**
- `TunnelMaster/Services/ServiceStore.swift` — Create
- `TunnelMaster/App/AppState.swift` — Add persistence integration
   ...
   actor ServiceStore {
       ...
       private var servicesURL: URL {
           FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
               .appendingPathComponent("TunnelMaster/services.json")
       }
...
**Verification:**
- Check `~/Library/Application Support/TunnelMaster/services.json` exists after adding a service
...
**Files to touch:**
- `TunnelMaster/Views/Settings/SettingsWindow.swift` — Create
- `TunnelMaster/Views/Settings/ServicesTab.swift` — Create (placeholder)
- `TunnelMaster/Views/Settings/TunnelTab.swift` — Create (placeholder)
- `TunnelMaster/Views/Settings/GeneralTab.swift` — Create (placeholder)
...
**Files to touch:**
- `TunnelMaster/Services/ConfigImporter/SingBoxParser.swift` — Create
- `TunnelMaster/Services/ConfigImporter/ConfigImporter.swift` — Create (protocol)
...
**Files to touch:**
- `TunnelMaster/Services/ConfigImporter/ClashParser.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/ConfigImporter/V2RayParser.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/ConfigImporter/URIParser.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Views/Settings/ServicesTab.swift` — Implement
- `TunnelMaster/Views/Settings/ServiceRow.swift` — Create
- `TunnelMaster/Views/Settings/ImportSheet.swift` — Create
...
**Files to touch:**
- Create new target: `TunnelMasterHelper`
- `Shared/XPCProtocol.swift` — Create (shared Swift package or framework)
- `TunnelMasterHelper/Info.plist` — Configure
- `TunnelMasterHelper/launchd.plist` — Create

**Implementation steps:**
1. In Xcode: File → New → Target → macOS → Command Line Tool
   - Name: `TunnelMasterHelper`
   - Bundle ID: `nniel.TunnelMaster.helper`
2. Create shared XPC protocol (can be in a local Swift package):
   ...
   public let helperMachServiceName = "nniel.TunnelMaster.helper"
   ```
3. Create `launchd.plist` in helper's Resources:
   ...
   <dict>
       <key>Label</key>
       <string>nniel.TunnelMaster.helper</string>
       <key>MachServices</key>
       <dict>
           <key>nniel.TunnelMaster.helper</key>
           <true/>
       </dict>
   </dict>
   </plist>
...
**Files to touch:**
- `TunnelMasterHelper/main.swift` — Implement XPC listener
- `TunnelMasterHelper/HelperDelegate.swift` — Create
...
**Files to touch:**
- `TunnelMasterHelper/SingBoxManager.swift` — Create
- `TunnelMasterHelper/HelperDelegate.swift` — Wire up
...
5. Log crashes to `~/Library/Logs/TunnelMasterHelper/`
...
**Files to touch:**
- `TunnelMaster/Services/XPCClient.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/HelperInstaller.swift` — Create
- `TunnelMaster/App/AppState.swift` — Add helper status
- `TunnelMaster/Views/Settings/GeneralTab.swift` — Add install UI
...
1. Create `HelperInstaller.swift`:
   ...
   class HelperInstaller {
       ...
       var isInstalled: Bool {
           SMAppService.daemon(plistName: "nniel.TunnelMaster.helper.plist").status == .enabled
       }

       func install() async throws {
           let service = SMAppService.daemon(plistName: "nniel.TunnelMaster.helper.plist")
...
**Verification:**
- `launchctl list | grep TunnelMaster` shows helper
...
**Files to touch:**
- `TunnelMasterHelper/ConfigBuilder.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Views/Settings/TunnelTab.swift` — Implement
- `TunnelMaster/Views/Settings/RuleRow.swift` — Create
- `TunnelMaster/Views/Settings/ChainEditor.swift` — Create
...
**Files to touch:**
- `TunnelMaster/App/AppState.swift` — Add connect/disconnect methods
- `TunnelMaster/Views/MenuBar/MenuBarView.swift` — Wire up toggle
- `TunnelMaster/Services/ConfigNormalizer.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/LatencyTester.swift` — Create
- `TunnelMaster/Views/Settings/ServiceRow.swift` — Add latency display
...
**Files to touch:**
- `TunnelMaster/Services/DockerManager.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/SSHClient.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Wizard/ProtocolTemplates/VLESSTemplate.swift` — Create
- `TunnelMaster/Wizard/ProtocolTemplates/TrojanTemplate.swift` — Create
- `TunnelMaster/Wizard/ProtocolTemplates/ShadowsocksTemplate.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Wizard/WizardView.swift` — Create
- `TunnelMaster/Wizard/Steps/TargetStep.swift` — Create
- `TunnelMaster/Wizard/Steps/ProtocolStep.swift` — Create
- `TunnelMaster/Wizard/Steps/ConfigureStep.swift` — Create
- `TunnelMaster/Wizard/Steps/DeployStep.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Wizard/RemoteDeployer.swift` — Create
...
**Files to touch:**
- `TunnelMaster/Services/ConfigExporter.swift` — Create
- `TunnelMaster/Views/Settings/ServicesTab.swift` — Add export button
...
**Files to touch:**
- `TunnelMaster/Services/GeoDatabaseUpdater.swift` — Create
- `TunnelMaster/Views/Settings/GeneralTab.swift` — Add update UI
...
**Files to touch:**
- `TunnelMaster/Views/Onboarding/OnboardingWindow.swift` — Create
- `TunnelMaster/App/TunnelMasterApp.swift` — Show on first launch
...
### Test Location
- `TunnelMasterTests/` — Unit tests
- `TunnelMasterUITests/` — UI tests (optional)
...
xcodebuild test -scheme TunnelMaster -only-testing:TunnelMasterTests/ServiceTests
```
